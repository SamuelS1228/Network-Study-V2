        ...
        with st.spinner("Optimizing warehouse locations..."):
            ...
            try:
                ...
                with tab1:
                    ...
                    # Create a basic scatterplot without projection
                    # Stores layer
                    stores_chart = alt.Chart(store_data).mark_circle().encode(
                        x=alt.X('Longitude:Q', scale=alt.Scale(domain=[-130, -65])),
                        y=alt.Y('Latitude:Q', scale=alt.Scale(domain=[23, 50])),
                        size=alt.Size('Sales:Q', scale=alt.Scale(range=[50, 500]), title='Sales'),
                        color=alt.Color('Cluster:N', title='Assigned Warehouse'),
                        tooltip=['Latitude', 'Longitude', 'Sales', 'Distance_km', 'Region']
                    ).properties(
                        width=800,
                        height=500
                    )
                    ...
                    *Note: Map is a simple scatterplot of coordinates (not a geographic projection)*
                    """)
